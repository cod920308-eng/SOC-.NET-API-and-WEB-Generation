...
namespace PrivateWeb.Pages
{
	public class EventsModel : PageModel
	{
		...
		public List<PrivateEvent> Events { get; set; } = new List<PrivateEvent>();
		...
		public async Task OnGetAsync()
		{
			...
			// Load all events
			HttpResponseMessage res = await client.GetAsync($"{ApiBase}/api/event");
			if (res.IsSuccessStatusCode)
			{
				string data = await res.Content.ReadAsStringAsync();
				Events = JsonSerializer.Deserialize<List<PrivateEvent>>(data,
					new JsonSerializerOptions { PropertyNameCaseInsensitive = true })
					?? new List<PrivateEvent>();
			}

			// Load this participant's registrations
			...
			if (participantId > 0 && UserRole == "Citizen")
			{
				try
				{
					...
					if (regRes.IsSuccessStatusCode)
					{
						...
						var regs = JsonSerializer.Deserialize<List<PrivateEventRegistration>>(regData,
							new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
